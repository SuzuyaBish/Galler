...
const GestureScrollView: React.FC<GestureScrollViewProps> = ({
  children,
  onHorizontalGesture,
  pagerEnabled = true,
  ...props
  ...
  const handleGesture = React.useCallback((direction: "left" | "right") => {
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      'worklet';
      if (hasTriggeredGesture.value || !pagerEnabled || isScrolling.value) return
      ...
      // Check velocity
      const isQuickSwipe = Math.abs(event.velocityX) > MIN_SWIPE_VELOCITY && 
                          Math.abs(event.velocityX) > Math.abs(event.velocityY) * VELOCITY_THRESHOLD
